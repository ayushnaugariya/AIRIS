...
Every adapter — mock, API-based, or scraped — must produce FareObservation
objects. This is the one shape the rest of the pipeline (validator, DB,
index construction) understands, regardless of where the data came from.
"""
...
class FareObservation(BaseModel):
    ...
    # --- the actual fare, decomposed (this feeds the Comparability Engine) ---
    ...
    # --- provenance — every observation must be traceable ---
    source: str = Field(..., description="Adapter name, e.g. 'amadeus', 'makemytrip'")
    scraped_at: datetime = Field(default_factory=datetime.utcnow)
    raw_payload_hash: str = Field(..., description="Hash of raw source payload for audit trail")

    # --- filled in by the validator, not the adapter ---
